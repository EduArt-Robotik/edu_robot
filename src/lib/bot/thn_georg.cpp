...
static ThnGeorg::Parameter get_robot_ros_parameter(rclcpp::Node& ros_node)
{
  ThnGeorg::Parameter parameter;
...
void ThnGeorg::initialize(eduart::robot::HardwareComponentFactory& factory)
{
  // Motor Controllers
...
Eigen::MatrixXf ThnGeorg::getKinematicMatrix(const DriveKinematic kinematic) const
{
  ...
  if (kinematic == DriveKinematic::MECANUM_DRIVE) {
    ...
    throw std::invalid_argument("ThnGeorg: given kinematic is not supported.");
  }
